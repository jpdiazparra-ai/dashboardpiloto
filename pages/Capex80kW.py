...
@st.cache_data(show_spinner=True, ttl=120)
def load_director_general_data(sheet_source_url: str, sheet_name: str = "Director General Técnico") -> pd.DataFrame:
...
@st.cache_data(show_spinner=True, ttl=120)
def load_valorizacion_data(url: str) -> pd.DataFrame:
...
@st.cache_data(show_spinner=True, ttl=120)
def load_dashboard_financiero_data(url: str) -> pd.DataFrame:
    df = pd.read_csv(url, dtype=str)
    df.columns = [str(c).strip() for c in df.columns]
...
def render_inputs_financial_main_kpis(df_in: pd.DataFrame):
    ...
    st.markdown(
        """
        <style>
        .inputs-fin-kpi-wrap{display:grid;grid-template-columns:repeat(4,minmax(220px,1fr));gap:16px;margin:10px 0 18px}
        @media (max-width:1200px){.inputs-fin-kpi-wrap{grid-template-columns:repeat(2,minmax(220px,1fr));}}
        @media (max-width:700px){.inputs-fin-kpi-wrap{grid-template-columns:1fr;}}
        .inputs-fin-kpi-card{
        ...
        .inputs-fin-kpi-row{display:flex;align-items:center;gap:10px;margin-bottom:10px}
        .inputs-fin-kpi-ico{
        ...
        .inputs-fin-kpi-h{font-size:13px;font-weight:800;color:#0f172a;letter-spacing:.02em}
        .inputs-fin-kpi-v{font-size:28px;font-weight:900;color:#0f172a;line-height:1.05;margin-bottom:8px}
        .inputs-fin-kpi-sub{display:flex;gap:8px;flex-wrap:wrap}
        .inputs-fin-chip{
    ...
    cards = f"""
    <div class="inputs-fin-kpi-wrap">
      <div class="inputs-fin-kpi-card">
        <div class="inputs-fin-kpi-row"><div class="inputs-fin-kpi-ico">💰</div><div class="inputs-fin-kpi-h">Monto Total (CLP)</div></div>
        <div class="inputs-fin-kpi-v">{html.escape(format_clp(monto_total))}</div>
        <div class="inputs-fin-kpi-sub">
          <span class="inputs-fin-chip">Base: {n_items:,} ítems</span>
          <span class="inputs-fin-chip">Proveedores: {n_prov:,}</span>
        </div>
      </div>
      <div class="inputs-fin-kpi-card">
        <div class="inputs-fin-kpi-row"><div class="inputs-fin-kpi-ico">🧮</div><div class="inputs-fin-kpi-h">Monto Promedio / ítem</div></div>
        <div class="inputs-fin-kpi-v">{html.escape(format_clp(monto_prom))}</div>
        <div class="inputs-fin-kpi-sub"><span class="inputs-fin-chip">Distribución por ítem</span></div>
      </div>
      <div class="inputs-fin-kpi-card">
        <div class="inputs-fin-kpi-row"><div class="inputs-fin-kpi-ico">📦</div><div class="inputs-fin-kpi-h">Ítems con Monto</div></div>
        <div class="inputs-fin-kpi-v">{n_items:,}</div>
        <div class="inputs-fin-kpi-sub"><span class="inputs-fin-chip">Con valor asignado</span></div>
      </div>
      <div class="inputs-fin-kpi-card">
        <div class="inputs-fin-kpi-row"><div class="inputs-fin-kpi-ico">🏭</div><div class="inputs-fin-kpi-h">Proveedores únicos</div></div>
        <div class="inputs-fin-kpi-v">{n_prov:,}</div>
        <div class="inputs-fin-kpi-sub"><span class="inputs-fin-chip">Diversidad de oferta</span></div>
      </div>
    </div>
    """
...
def render_inputs_sm_kpi_cards(tabla_sm: pd.DataFrame):
    ...
    st.markdown(
        """
        <style>
        .inputs-smkpi-grid{display:grid;grid-template-columns:repeat(3,minmax(220px,1fr));gap:16px;margin:0 0 12px}
        @media (max-width:1000px){.inputs-smkpi-grid{grid-template-columns:1fr;}}
        .inputs-smkpi-card{
            border-radius:16px;padding:14px 16px;background:linear-gradient(180deg,#f8fafc 0%,#ffffff 62%);
            border:1px solid rgba(148,163,184,.35);box-shadow:0 4px 10px rgba(15,23,42,.05)
        }
        .inputs-smkpi-title{font-size:14px;font-weight:700;color:#0f172a;margin-bottom:6px}
        .inputs-smkpi-value{font-size:28px;font-weight:800;color:#0f172a;margin:2px 0 6px}
        .inputs-smkpi-row{font-size:13px;color:#475569;display:flex;gap:8px;flex-wrap:wrap}
        ...
        .inputs-smkpi-bar{position:relative;width:100%;height:8px;border-radius:999px;background:#eef2f7;margin-top:10px}
...
def load_project_gantt_data(url: str) -> pd.DataFrame:
...
def render_inputs_project_gantt():
    try:
        df_gantt = load_project_gantt_data(GANTT_PROJECT_CSV_URL_DEFAULT)
    except Exception as exc:
...
def render_inputs_estado_actual_dashboard():
    try:
        df_fin = load_dashboard_financiero_data(DASHBOARD_FINANCIERO_CSV_URL_DEFAULT)
    except Exception as exc:
...
try:
    df_direccion = load_director_general_data(capex_url)
...
def render_resumen_content(
    ...
    fig_item_total = px.bar(
        ...
        text="Texto",
        color="Item",
        color_discrete_map={**item_color_map, "Dirección": "#0F766E"},
        labels={"Total_MM": "Monto (millones de CLP)", "Item": "Ítem"},
        title=(
            "CAPEX + Dirección por ítem (monto total y % del total integrado)"
...
def render_valorizacion_module_content(key_prefix: str = "val_"):
    ...
    bloque_cards = [
        ("1. Bases de valorización", "1-Fundamentos de Valor del Proyecto"),
        ("2. Revalorización post piloto", "2-Etapa de Inversión Inicial – Validación y calamiento (Serie A)"),
        ("3. Valorización post piloto operativo", "3-Escenario de Valor Post Validación Tecnológica"),
        ("4. Segunda ronda y estructura accionaria", "4-Escalamiento Comercial y Expansión (Serie B)"),
    ]
    if state_bootstrap_key not in st.session_state:
        st.session_state[state_block_key] = bloque_cards[0][0]
    ...
    try:
        df_valorizacion = load_valorizacion_data(VALORIZACION_CSV_URL_DEFAULT)
    ...
    try:
        df_eerrv2 = load_eerrv2_data(EERRV2_CSV_URL_DEFAULT)
        eerrv2_error = None
    except Exception as exc:
    ...
    fx_default = parse_model_number(model_map.get("fxclpusd", 915))
    total_base_knowhow_clp = parse_model_number(model_map.get("totalbaseinversionknowhow", 0))
    pre_money_actual_default = total_base_knowhow_clp / fx_default if fx_default > 0 and total_base_knowhow_clp > 0 else 0.0
    inversion_clp_default = parse_model_number(model_map.get("inversionimelsaparadesarrollodelpiloto80kwclp", 0))
    ...
    if bloque_sel == "1. Bases de valorización":
        ...
        with solo_kpi:
            kpi_card(
                "VALORIZACIÓN FLUXIAL HOY (PRE-MONEY)",
                format_usd(valorizacion_fluxial_preview),
                "Base en USD más EBITDA potencial del ciclo inicial multiplicado por el múltiplo visible.",
            )
    ...
    if bloque_sel == "1. Bases de valorización":
        ...
        with mk2:
            kpi_card("Valorización post piloto", format_usd(valor_post_piloto), "EBITDA × múltiplo × captura.")
        with mk3:
        ...
        sensibilidad["Valorización_post_piloto"] = sensibilidad["Volumen"] * ebitda_unit_input * multiple_input * captura_input
        ...
        with mk3:
            kpi_card("Post-money Serie B", format_usd(post_money_serie_b), "Valorización posterior al cierre de la ronda.")
        with mk4:
...
# =========================
input_cards = [
    ("estado_actual", "1-Estado del Activo y Nivel de Madurez Tecnológica"),
...
if "inputs_bloque_sel" not in st.session_state:
    st.session_state["inputs_bloque_sel"] = input_cards[0][0]
...
st.markdown("---")
selected_input_block = st.session_state.get("inputs_bloque_sel", input_cards[0][0])

input_block_copy = {
    "estado_actual": (
        "1-Estado del Activo y Nivel de Madurez Tecnológica",
